...
# Chroma will be our database rather than Pinecone (since it's a smaller app)
import os
...
def load_document(file):
    ...
    if extension == '.pdf':
        ...
        # We are importing here to prevent circular dependency.
...
def ask_and_get_answer(vector_store, q, k=3):
    ...
    chain = RetrievalQA.from_chain_type(llm=llm,
                                        chain_type="stuff",
                                        retriever=retriever)
    # "stuff" is the default and uses all of the text
    answer = chain.run(q)
    return answer
...
if __name__ == "__main__":
    ...
    st.image('img.png')
    st.subheader("LLM Q&A Application :dragon:")
    with st.sidebar:
